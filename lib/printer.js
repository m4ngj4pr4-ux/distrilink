...
class PrinterService {
  ...
  formatReceipt(transaction) {
    ...
    addStr(`Tgl   : ${dateStr}\n`);
    addStr(`Sales : ${namaSales.substring(0, 24)}\n`);
    add(PRINTER_COMMANDS.ALIGN_CENTER);
    addStr("================================\n");

    // Items Header
    add(PRINTER_COMMANDS.ALIGN_LEFT);
    addStr("Item          Qty      Subtotal\n");
    
    // Item Detail
    const namePart = productName.substring(0, 14).padEnd(14);
    const qtyPart = `${jumlahDrop}x`.padEnd(8);
    const subPart = (jumlahDrop * (hargaJual || 0)).toLocaleString('id-ID').padStart(10);
    addStr(`${namePart}${qtyPart}${subPart}\n`);
    ...
    const totalVal = (total || (jumlahDrop * hargaJual)).toLocaleString('id-ID').padStart(10);
    addStr(`${totalLabel}${totalVal}\n`);
    add(PRINTER_COMMANDS.BOLD_OFF);
    
    add(PRINTER_COMMANDS.ALIGN_CENTER);
    addStr("================================\n");
    addStr("Terima Kasih\n");
    addStr("Barang yang sudah dibeli\n");
    addStr("tidak dapat ditukar/kembali.\n");
