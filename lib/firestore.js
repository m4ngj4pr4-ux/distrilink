...
export async function updatePO(poId, data) {
  const financeQuery = query(collection(db, "finance_ledger"), where("relasiId", "==", poId));
  const financeSnap = await getDocs(financeQuery);

  const paymentsQuery = query(collection(db, "factory_payments"), where("purchaseId", "==", poId));
  ...
  return runTransaction(db, async (transaction) => {
    ...
    // Sync DP in factory_payments & finance_ledger if changed or if date changed
    const newDP = data.uangMuka !== undefined ? data.uangMuka : (oldData.uangMuka || 0);
    const oldDP = oldData.uangMuka || 0;

    const dpPaymentDoc = paymentsSnap.docs.find(d => d.data().isDP === true);
    const dpLedgerDoc = financeSnap.docs.find(d => d.data().tipeBuku === "auto_bayar_po" && d.data().keterangan?.includes("DP"));

    const newDateVal = data.createdAt;
    const oldDateVal = oldData.createdAt;
    ...
    const isDateChanged = newDateVal && oldDateVal && getMillis(newDateVal) !== getMillis(oldDateVal);

    if (newDP !== oldDP) {
      if (newDP > 0) {
        // Create or Update DP in factory_payments
        if (dpPaymentDoc) {
          transaction.update(dpPaymentDoc.ref, {
            amount: newDP,
            createdAt: newDateVal || oldDateVal || serverTimestamp()
          });
        } else {
          const payRef = doc(collection(db, "factory_payments"));
          transaction.set(payRef, {
        ...
        // Create or Update DP in finance_ledger
        if (dpLedgerDoc) {
          transaction.update(dpLedgerDoc.ref, {
            nominal: newDP,
            keterangan: `[Auto] DP / Pembayaran Awal PO #${poId.slice(-6).toUpperCase()}`,
            createdAt: newDateVal || oldDateVal || serverTimestamp()
          });
        } else {
          const ledgerRef = doc(collection(db, "finance_ledger"));
        ...
        // Delete DP documents if set to 0
        if (dpPaymentDoc) {
          transaction.delete(dpPaymentDoc.ref);
        }
        if (dpLedgerDoc) {
          transaction.delete(dpLedgerDoc.ref);
        }
      }
    } else if (isDateChanged) {
      // Nominal DP is unchanged but the PO transaction date has changed - sync the timestamps!
      if (dpPaymentDoc) {
        transaction.update(dpPaymentDoc.ref, { createdAt: newDateVal });
      }
      if (dpLedgerDoc) {
        transaction.update(dpLedgerDoc.ref, { createdAt: newDateVal });
      }
    }
  });
}
...
export async function recalculateSummary() {
  ...
  let productAdminDistributedPacksMap = {};

  productSnap.forEach(p => {
    productAdminDistributedPacksMap[p.id] = 0;
  });
  ...
  distSnap.forEach(d => {
     ...
     if (data.productId) {
       productAdminDistributedPacksMap[data.productId] = (productAdminDistributedPacksMap[data.productId] || 0) + (data.totalPacksDistributed || 0);
     }
  ...
  returnsSnap.forEach(d => {
    ...
    if (data.productId) {
      productAdminDistributedPacksMap[data.productId] = (productAdminDistributedPacksMap[data.productId] || 0) - Math.abs(data.totalPacksReturned || 0);
    }
  ...
  // Karena semua DP sudah disinkronkan ke factory_payments, kita cukup menjumlahkan factory_payments.
  ...
  const kasOut = totalBayarPabrik + totalBagiHasil + totalBiayaOperasional + totalPengeluaranLain;
  const recalcModalAktif = kasIn - kasOut;
  ...
  for (const p of productSnap.docs) {
      ...
      await updateDoc(doc(db, "products", p.id), {
          totalPacks: newPacks,
          adminDistributedPacks: Math.max(0, productAdminDistributedPacksMap[p.id] || 0),
...
export async function payFactoryDebt(purchaseId, amount, keterangan = "", potongKas = true) {
  const poRef = doc(db, "purchases", purchaseId);
  const SUMMARY_DOC = doc(db, "summary", "dashboard");
  ...
  return runTransaction(db, async (transaction) => {
    ...
    if (summarySnap.exists()) {
      ...
      // AUTO-JOURNAL: Bayar hutang pabrik mengurangi kas HANYA JIKA potongKas = true
      if (potongKas) {
        // We do not cap to 0 here to allow negative balance if overdrawn, 
        // but previously it was max(0). Let's keep it accurately decreasing.
        updates.modalAktif = (sData.modalAktif || 0) - amount;
      }
      transaction.update(SUMMARY_DOC, updates);
    }
    ...
    transaction.set(payRef, {
      purchaseId,
      amount,
      keterangan: keterangan || (potongKas ? "" : "Penyesuaian (Tidak Potong Kas)"),
      potongKas,
      createdAt: serverTimestamp()
    });
    ...
    if (potongKas) {
      ...
      transaction.set(logRef, {
        tipeBuku: "auto_bayar_po",
        nominal: amount,
        keterangan: keterangan ? `[Auto] ${keterangan} (PO #${purchaseId.slice(-6).toUpperCase()})` : `[Auto] Pembayaran Hutang PO #${purchaseId.slice(-6).toUpperCase()}`,
        relasiId: purchaseId,
        isAutoJournal: true,
        createdAt: serverTimestamp()
      });
  ...
  return runTransaction(db, async (transaction) => {
    ...
    if (productSnap?.exists()) {
      ...
      transaction.update(productRef, {
        ...
        adminDistributedPacks: Math.max(0, (pData.adminDistributedPacks || 0) - totalPacksDistributed),
        updatedAt: serverTimestamp()
      });
    }
  ...
  return runTransaction(db, async (transaction) => {
    ...
    if (productSnap.exists()) {
      transaction.update(productRef, {
        ...
        adminDistributedPacks: Math.max(0, (productSnap.data().adminDistributedPacks || 0) - Math.abs(totalPacksReturned)),
        updatedAt: serverTimestamp()
      });
    }
...
export async function deleteReturnTransaction(returnId, returnData) {
...
export async function addFactoryReturnTransaction(data) {
  const { productId, totalPacksReturned, returnAmount, purchaseId } = data;
  const productRef = doc(db, "products", productId);
  const SUMMARY_DOC = doc(db, "summary", "dashboard");
  ...
  return runTransaction(db, async (transaction) => {
    ...
    let poSnap = null;
    if (purchaseId) {
      poRef = doc(db, "purchases", purchaseId);
    ...
    if (poSnap && poSnap.exists()) {
      transaction.update(poRef, {
        sisaHutang: Math.max(0, (poSnap.data().sisaHutang || 0) - Math.abs(returnAmount)),
        updatedAt: serverTimestamp()
      });

      // Log retur as payment for the PO
      const payRef = doc(collection(db, "factory_payments"));
      transaction.set(payRef, {
        purchaseId,
        amount: Math.abs(returnAmount),
        keterangan: "Retur Barang ke Pabrik",
        isRetur: true,
        createdAt: serverTimestamp()
      });
    }
...
export async function deleteFactoryReturnTransaction(returnId, returnData) {
  const { productId, totalPacksReturned, returnAmount, purchaseId } = returnData;
  const productRef = doc(db, "products", productId);
  const SUMMARY_DOC = doc(db, "summary", "dashboard");
  const returnRef = doc(db, "factory_returns", returnId);
  ...
  return runTransaction(db, async (transaction) => {
    ...
    if (summarySnap.exists()) {
      transaction.update(SUMMARY_DOC, {
        factoryDebt: (summarySnap.data().factoryDebt || 0) + Math.abs(returnAmount),
        totalAssets: (summarySnap.data().totalAssets || 0) + Math.abs(returnAmount),
        updatedAt: serverTimestamp()
      });
    }
    ...
    if (poSnap && poSnap.exists()) {
      transaction.update(poRef, {
        sisaHutang: (poSnap.data().sisaHutang || 0) + Math.abs(returnAmount),
        updatedAt: serverTimestamp()
      });
      
      // Note: We are not explicitly deleting the factory_payment log because it's hard to find 
      // without storing its ID. But the overall PO debt is restored, which is what matters most.
    }
...
export async function getSalesUser(userId) {
  const docRef = doc(db, "sales_ledger", userId);
...
export async function addStoreReturnTransactionBatch(items, storeInfo, agentInfo) {
  const batch = writeBatch(db);
  const time = serverTimestamp();
  
  // Format receiptId: RET-YYYYMMDD-XXXX
  const today = new Date();
  const dateStr = today.getFullYear().toString() + 
                  (today.getMonth() + 1).toString().padStart(2, '0') + 
                  today.getDate().toString().padStart(2, '0');
  const randomStr = Math.floor(1000 + Math.random() * 9000).toString();
  const receiptId = `RET-${dateStr}-${randomStr}`;
  ...
  for (const result of checkResults) {
    const { item, invRef, exists } = result;
    const qty = parseInt(item.jumlahDrop) || 0; // The quantity being returned
    const price = parseInt(item.hargaJual) || 0;

    // Create a new sales transaction document with tipe: 'retur_toko'
    const txRef = doc(collection(db, "sales_transactions"));
    batch.set(txRef, {
      teamId: agentInfo.id,
      namaSales: agentInfo.name,
      ...
      productId: item.productId,
      productName: item.productName,
      jumlahDrop: qty, // Keep positive here because we handle the logic in getSisaStokSales
      hargaJual: price,
      total: qty * price,
      catatan: item.catatan || "Penarikan (Retur) dari Toko",
      receiptId: receiptId,
      tipe: 'retur_toko',
      waktu: time
    });
    ...
    if (exists) {
      batch.update(invRef, {
        currentStock: increment(-qty), // Reduce store inventory
        lastReturnBy: agentInfo.name,
        lastReturnAt: time
      });
    }
  }
...
export async function getSisaStokSales(teamId) {
  try {
    ...
    snapDrop.forEach((doc) => {
      const tx = doc.data();
      if (tx.tipe === "retur_toko") {
        totalDropped -= (tx.jumlahDrop || 0);
      } else {
        totalDropped += (tx.jumlahDrop || 0);
      }
    });
...
export async function getGroupedStoreInventory(teamId) {
  try {
    // 1. Ambil semua transaksi drop sales ini (hanya tipe 'drop' dan 'retur_toko')
    const qTx = query(
      collection(db, "sales_transactions"), 
      where("teamId", "==", teamId),
      where("tipe", "in", ["drop", "retur_toko"])
    );
    ...
    // 1b. Ambil toko terdaftar milik sales ini ATAU yang diinput oleh web admin (tanpa teamId)
    // agar toko baru dan toko admin/dummy tetap muncul
    const snapStores = await getDocs(collection(db, "retail_stores"));
    
    // Kumpulkan ID toko yang MASIH ADA di database dan relevan untuk sales ini
    const activeStoreIds = new Set();
    snapStores.forEach(d => {
      const store = d.data();
      const sid = d.id;
      // Relevan jika milik sales ini ATAU dibuat oleh admin (falsy teamId)
      if (store.teamId === teamId || !store.teamId) {
...
// Menolak setoran dana (status berubah menjadi Ditolak)
export async function rejectSetoran(id) {
  const docRef = doc(db, "setoran_dana", id);
  return updateDoc(docRef, {
    status: "Ditolak",
    updatedAt: serverTimestamp()
...
export async function getAgentPerformanceData(teamId, role) {
  try {
    ...
    // Ambil data retur sales ini
    const qRet = query(collection(db, "returns"), where("teamId", "==", teamId));
    const snapRet = await getDocs(qRet);
    const returnsData = snapRet.docs.map(d => d.data());
    const totalRetur = returnsData.reduce((s, r) => s + (r.totalPacksReturned || 0), 0);
    ...
    if (role === 'captain') {
      ...
      bawaanNetto = Math.max(0, grossReceived - totalDioper - totalRetur);
    } else {
      bawaanNetto = Math.max(0, distributions.reduce((s, d) => s + (d.totalPacksDistributed || 0), 0) - totalRetur);
    }
  ...
  try {
    ...
    snapSetor.forEach(doc => {
      ...
      if (data.status === "Ditolak") {
        ledgerItems.push({
          ...
          keterangan: `Bayar (${data.metode || 'Transfer'}) [Ditolak]` + (data.catatan ? ` - ${data.catatan}` : ''),
          qty: '',
          harga: '',
          tipe: 'setoran_rejected',
          nilai: -Math.abs(data.nominal || 0),
        });
      } else if (data.status === "Menunggu Diterima Captain") {
        ledgerItems.push({
          id: doc.id,
          tanggal: data.waktu || data.createdAt || null,
    ...
    snapRet.forEach(doc => {
      ...
      ledgerItems.push({
        ...
        qty: `${data.qtyOriginal || 0} ${data.unit || 'Pk'}`,
        harga: data.pricePerPack || data.hppSnapshot || 0,
        tipe: 'retur',
    ...
    const ledgerWithBalance = ledgerItems.map(item => {
      if (item.tipe !== 'setoran_pending' && item.tipe !== 'setoran_rejected') {
...
// CENTRALIZED HELPER: Calculate real remaining stock for PO batches based on net distributions
export function calculatePOBatchesWithRealSisa(purchases = [], distributions = [], returns = [], factoryReturns = []) {
  ...
  const distClones = safeDistributions.map(d => ({ ...d }));

  // Sort all returns chronologically (OLDEST to NEWEST)
  const sortedReturns = [...safeReturns];
  ...
  // Group net distributions by poId
  const distByPO = {};
  const unassignedDistributions = {}; // key: productId, value: total packs

  distClones.forEach(d => {
    if (d.poId) {
      distByPO[d.poId] = (distByPO[d.poId] || 0) + (d.totalPacksDistributed || 0);
    } else {
      unassignedDistributions[d.productId] = (unassignedDistributions[d.productId] || 0) + (d.totalPacksDistributed || 0);
    }
  });

  // Treat factory returns as distributions that reduce warehouse stock
  const safeFactoryReturns = Array.isArray(factoryReturns) ? factoryReturns : [];
  safeFactoryReturns.forEach(fr => {
    const qty = Math.abs(fr.totalPacksReturned || 0);
    if (fr.purchaseId) {
      distByPO[fr.purchaseId] = (distByPO[fr.purchaseId] || 0) + qty;
    } else {
      unassignedDistributions[fr.productId] = (unassignedDistributions[fr.productId] || 0) + qty;
    }
  });

  // Sort purchases OLDEST to NEWEST to deduct unassigned distributions FIFO
  const sortedPurchases = safePurchases
    .filter(po => po && po.status !== "pengiriman")
    .sort((a, b) => getMillis(a.createdAt) - getMillis(b.createdAt));

  // Map each PO to include its realSisa
  const resultBatches = sortedPurchases.map(po => {
    let distributed = distByPO[po.id] || 0;
    
    // Deduct unassigned distributions (FIFO)
    if (unassignedDistributions[po.productId] && unassignedDistributions[po.productId] > 0) {
      const sisaBeforeUnassigned = (po.totalPack || 0) - distributed;
      if (sisaBeforeUnassigned > 0) {
        const deduct = Math.min(sisaBeforeUnassigned, unassignedDistributions[po.productId]);
        distributed += deduct;
        unassignedDistributions[po.productId] -= deduct;
      }
    }

    const realSisa = Math.max(0, (po.totalPack || 0) - distributed);
    return {
      ...po,
      realSisa
    };
