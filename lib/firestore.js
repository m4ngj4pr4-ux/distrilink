  ...
  if (data.tipeBuku === "bagi_hasil" || data.tipeBuku === "biaya_operasional" || data.tipeBuku === "pengeluaran_lain") {
    return runTransaction(db, async (transaction) => {
      ...
      if (summarySnap.exists()) {
        ...
        // Hanya bagi_hasil yang mengurangi sisa laba (ambil dari profit pool)
        if (isBagiHasil) {
  ...
  if (data?.tipeBuku === "bagi_hasil" || data?.tipeBuku === "biaya_operasional" || data?.tipeBuku === "pengeluaran_lain") {
    return runTransaction(db, async (transaction) => {
      ...
      if (summarySnap.exists()) {
        ...
        // Hanya bagi_hasil yang mengembalikan sisa laba
        if (isBagiHasil) updates.sisaLabaBelumDibagikan = (sData.sisaLabaBelumDibagikan || 0) + data.nominal;

        if (isOps || data.tipeBuku === "pengeluaran_lain") {
          updates.modalAktif = (sData.modalAktif || 0) + data.nominal; // kembalikan kas
        }
