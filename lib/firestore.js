...
export async function syncProductPacks(products) {
  for (const p of products) {
    // Jika totalPacks belum ada ATAU bernilai 0, tapi ada stok karton lama
    if ((p.totalPacks === undefined || p.totalPacks === 0) && p.stockCartons > 0) {
      ...
      console.log(`Migrasi stok untuk ${p.name}: ${totalPacks} packs`);
...
export async function addGoodsDropTransaction(data) {
  ...
  // 2. Kurangi stok produk (satuan Pack)
  if (productId) {
