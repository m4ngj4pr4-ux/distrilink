...
export async function recalculateSummary() {
  // 1. Hitung total Hutang Pabrik & Aset dari seluruh PO yang tersisa
  const purchaseSnap = await getDocs(purchasesCol);
  let totalDebt = 0;
  let totalPOValue = 0;
  ...
  // 2. Hitung total Piutang Sales dari seluruh Tim yang tersisa
  ...
  // 3. Update doc summary dengan angka segar
  // Total Aset = Nilai PO yang masuk + Piutang yang beredar (sebagai representasi nilai operasional)
  // Jika semua dihapus, maka semua akan menjadi 0
  ...
  console.log("Recalculation complete:", { totalPOValue, totalDebt, totalReceivables });
}
