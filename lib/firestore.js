...
export async function updatePO(poId, data) {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    const oldSlopsPerKarton = ((oldData.conversion?.slopsPerBall || 20) * (oldData.conversion?.ballsPerKarton || 5)) + (oldData.conversion?.ekstraSlopPerKarton || 0);
    ...
    if (newPacks === undefined) {
      ...
      const newSlopsPerKarton = ((data.conversion?.slopsPerBall || oldData.conversion?.slopsPerBall || 20) * 
                                 (data.conversion?.ballsPerKarton || oldData.conversion?.ballsPerKarton || 5)) + 
                                 (data.conversion?.ekstraSlopPerKarton !== undefined ? data.conversion.ekstraSlopPerKarton : (oldData.conversion?.ekstraSlopPerKarton || 0));
      const newJumlahKarton = data.jumlahKarton !== undefined ? data.jumlahKarton : (oldData.jumlahKarton || 0);
      newPacks = newJumlahKarton * newSlopsPerKarton * newPacksPerSlop;
    }
...
export async function deletePurchase(purchaseId, data) {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    // Finance ledger mirrored records are no longer used for POs.
...
export async function getSalesStockBreakdown(teamId) {
  try {
    ...
    const result = Object.values(assigned).map(item => {
      const currentStock = Math.max(0, item.total - (dropped[item.productId] || 0));
      return {
        ...item,
...
export async function addFinanceEntry(data) {
  ...
  // Bagi hasil & biaya ops: update summary laba + modalAktif
  if (data.tipeBuku === "bagi_hasil" || data.tipeBuku === "biaya_operasional") {
    return runTransaction(db, async (transaction) => {
      ...
      if (summarySnap.exists()) {
        ...
        // Biaya operasional mengurangi kas
        if (isOps) {
  ...
  // Modal masuk: tambah kas
  if (data.tipeBuku === "modal_masuk") {
    return runTransaction(db, async (transaction) => {
      const summarySnap = await transaction.get(summaryRef);
      if (summarySnap.exists()) {
        const sData = summarySnap.data();
...
export async function syncFinancialBaseline() {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    transaction.update(SUMMARY_DOC, {
      ...
      factoryDebt: 97979505,         // Rp 97.979.505
    ...
    transaction.set(logRef, {
      ...
      nominal: 0,
      keterangan: "Sinkronisasi Saldo Awal (Historical PO & Sales) — Modal: Rp 72.000.000, Hutang: Rp 97.979.505",
      isAutoJournal: true,
      isBaseline: true,
      baseline: {
        modalAktif: 72000000,
        factoryDebt: 97979505
      },
      createdAt: serverTimestamp()
    });
  });
