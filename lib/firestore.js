...
export async function deleteSalesTeam(teamId, teamData) {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    // Kelompokkan data rollback
    const captainsToUpdate = {};
    const productsToUpdate = {};

    for (const d of snapDist.docs) {
      const data = d.data();
      if (data.source === "captain" && data.distributedByCaptainId) {
        // Balikkan hutang ke Captain
        captainsToUpdate[data.distributedByCaptainId] = (captainsToUpdate[data.distributedByCaptainId] || 0) + (data.amount || 0);
      } else {
        // Balikkan stok ke Gudang (Admin) & Kurangi Total Piutang Perusahaan
        if (data.productId) {
          productsToUpdate[data.productId] = (productsToUpdate[data.productId] || 0) + (data.totalPacksDistributed || 0);
        }
        newSalesReceivables = Math.max(0, newSalesReceivables - (data.amount || 0));
      }
    }

    // Ambil semua referensi Captain yang terkait
    const captainSnaps = {};
    for (const cId of Object.keys(captainsToUpdate)) {
      captainSnaps[cId] = await transaction.get(doc(db, "sales_ledger", cId));
    }

    // Ambil semua referensi Produk yang terkait
    const productSnaps = {};
    ...
    // Hapus histori distribusi ke sales ini
    for (const d of snapDist.docs) {
...
export async function deleteDistribution(distId, data) {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    // Reduce Summary's salesReceivables
    if (summarySnap.exists()) {
      const sData = summarySnap.data();
      transaction.update(SUMMARY_DOC, {
