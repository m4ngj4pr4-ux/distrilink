...
export async function addSetoranDana(data) {
...
export async function getCountPendingSetoran() {
  try {
    const q = query(collection(db, "setoran_dana"), where("status", "==", "Menunggu Verifikasi"));
    const snapshot = await getDocs(q);
...
export async function getGroupedStoreInventory(teamId) {
  try {
    // 1. Ambil semua transaksi drop sales ini
...
export async function captainDistributeStock({ captainId, captainName, targetTeamId, targetTeamName, productId, productName, totalPacks, pricePerPack }) {
...
// Ambil setoran "Menunggu Verifikasi" dari semua anggota tim
