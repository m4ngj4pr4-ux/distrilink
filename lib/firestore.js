...
export async function recalculateSummary() {
  ...
  // sisaLabaBelumDibagikan = laba kotor - yang sudah dibagikan saja
  const sisaLaba = accumulatedProfit - totalBagiHasil;

  // modalAktif = semua kas masuk - semua kas keluar
...
export async function editDropTransaction(txId, oldData, newData) {
  ...
  return runTransaction(db, async (transaction) => {
    ...
    if (isProductChanged) {
      // a. Kurangi stok barang lama
      if (oldData.storeId && oldData.productName) {
        const oldInvId = `${oldData.storeId}_${oldData.productName}`;
        const oldInvRef = doc(db, "store_inventory", oldInvId);
        const oldSnap = await transaction.get(oldInvRef);
        if (oldSnap.exists()) {
      ...
      // b. Tambahkan stok barang baru
      if (newData.storeId && newData.productName) {
        const newInvId = `${newData.storeId}_${newData.productName}`;
        const newInvRef = doc(db, "store_inventory", newInvId);
        const newSnap = await transaction.get(newInvRef);
        if (newSnap.exists()) {
  ...
  if (data.tipeBuku === "bagi_hasil" || data.tipeBuku === "biaya_operasional" || data.tipeBuku === "pengeluaran_lain") {
    return runTransaction(db, async (transaction) => {
      ...
      if (summarySnap.exists()) {
        ...
        // Biaya operasional dan Pengeluaran Lain mengurangi kas
        if (isOps || data.tipeBuku === "pengeluaran_lain") {
          updates.modalAktif = Math.max(0, (sData.modalAktif || 0) - data.nominal);
        }
  ...
  if (data?.tipeBuku === "bagi_hasil" || data?.tipeBuku === "biaya_operasional" || data?.tipeBuku === "pengeluaran_lain") {
    return runTransaction(db, async (transaction) => {
      ...
      if (summarySnap.exists()) {
        ...
        // Biaya operasional dan Pengeluaran Lain: kembalikan kas
        if (isOps || data.tipeBuku === "pengeluaran_lain") {
          updates.modalAktif = (sData.modalAktif || 0) + data.nominal;
        }
